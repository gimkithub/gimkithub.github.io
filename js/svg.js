/* Navbar */
const SUN_SVG = `
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 800 800" fill="none" stroke="var(--text2)" stroke-width="100">
            <circle cx="400" cy="400" r="125" fill="var(--text2)" />
            <path d="M735,436.3l-57.5,57.5c-9.6,9.6-15,22.7-15,36.3v81.3c0,28.3-22.9,51.3-51.2,51.3h-81.3c-13.6,0-26.6,5.4-36.2,15l-57.5,57.5c-20,20-52.4,20-72.5,0l-57.5-57.5c-9.6-9.6-22.6-15-36.2-15h-81.3c-28.3,0-51.2-23-51.2-51.3v-81.3c0-13.6-5.4-26.6-15-36.3l-57.5-57.5c-20-20-20-52.5,0-72.5l57.5-57.5c9.6-9.6,15-22.7,15-36.3v-81.3c0-28.3,22.9-51.3,51.2-51.3h81.3c13.6,0,26.6-5.4,36.2-15l57.5-57.5c20-20,52.4-20,72.5,0l57.5,57.5c9.6,9.6,22.6,15,36.2,15h81.3c28.3,0,51.2,23,51.2,51.3v81.3c0,13.6,5.4,26.6,15,36.3l57.5,57.5c20,20,20,52.5,0,72.5Z"/>
        </svg>
    `;

const MOON_SVG = `
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 800 800" fill="none" stroke="var(--text2)" stroke-width="100">
        <path d="M747.7,409.8c1.6.9,2.4,3.7,2.6,7.9-2.1,63.7-24,125.1-57.3,175.6-88.7,137.6-279,198.3-429.4,127.3-101.2-47.7-185.9-145.9-207.4-256.5C16.6,256.4,174.3,60.3,380,50.3c12.3-1,20.9,1.2,8.6,11.6-31.7,26.8-57.1,60.6-72.1,99.4-49.3,124,19,263.9,140.9,310.5,92.5,38.1,208.4,14.7,276.6-53.6,3.8-3.3,10.1-10.3,13.7-8.4Z" />
    </svg>
`;

/* Player Toolbar */
const FAVORITE_SVG = `
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 800 800" fill="#E10" stroke-width="100" id="heart_svg">
        <path class="b" d="M750.8,278.8c0,58.6-20,117.1-60.1,161.8l-262.7,292.8c-15,16.7-40.2,16.7-55.2,0L110.1,440.6c-80.2-89.4-80.2-234.2,0-323.6,40.1-44.7,92.6-67,145.1-67s105.1,22.4,145.1,67c80.1-89.3,210.1-89.3,290.3,0,40.1,44.7,60.1,103.2,60.1,161.8Z"/>
    </svg>
`;

const FULLSCREEN_SVG = `
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 800 800" fill="var(--text)">
        <path class="b" d="M374,471.6l-40,149.2c-7.4,27.6-41.9,36.9-62.2,16.7l-19.9-19.9-168,168c-19.2,19.2-50.3,19.2-69.5,0-19.2-19.2-19.2-50.3,0-69.5l168-168-19.9-19.9c-20.2-20.2-11-54.8,16.7-62.2l149.2-40c27.6-7.4,52.9,17.9,45.5,45.5Z"/>
        <path class="b" d="M471.6,426l149.2,40c27.6,7.4,36.9,41.9,16.7,62.2l-19.9,19.9,168,168c19.2,19.2,19.2,50.3,0,69.5-19.2,19.2-50.3,19.2-69.5,0l-168-168-19.9,19.9c-20.2,20.2-54.8,11-62.2-16.7l-40-149.2c-7.4-27.6,17.9-52.9,45.5-45.5Z"/>
        <path class="b" d="M426,328.4l40-149.2c7.4-27.6,41.9-36.9,62.2-16.7l19.9,19.9L716.1,14.4c19.2-19.2,50.3-19.2,69.5,0,19.2,19.2,19.2,50.3,0,69.5l-168,168,19.9,19.9c20.2,20.2,11,54.8-16.7,62.2l-149.2,40c-27.6,7.4-52.9-17.9-45.5-45.5Z"/>
        <path class="b" d="M328.4,374l-149.2-40c-27.6-7.4-36.9-41.9-16.7-62.2l19.9-19.9L14.4,83.9C-4.8,64.7-4.8,33.6,14.4,14.4c19.2-19.2,50.3-19.2,69.5,0l168,168,19.9-19.9c20.2-20.2,54.8-11,62.2,16.7l40,149.2c7.4,27.6-17.9,52.9-45.5,45.5Z"/>
    </svg>
`;

const SHARE_SVG = `
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 800 800" fill="var(--text)">
        <polyline class="c" fill="none" stroke="var(--text)" stroke-width=100 points="610 125 125 400 610 675"/><circle class="b" cx="125" cy="400" r="125"/><circle class="b" cx="610" cy="125" r="125"/><circle class="b" cx="610" cy="675" r="125"/>
    </svg>
`;

const REPORT_SVG = `
    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 650 800" stroke="var(--text)" stroke-width="100" stroke-linecap="round">
    <line class="c" x1="50" y1="50" x2="50" y2="750"/>
    <path class="b" d="M600,549.7l-128.3,12.7c-55,5.4-110.2-8.2-156.3-38.6h0c-46.9-30.9-103-44.5-158.8-38.4l-106.5,11.6V141.8l106.5-11.6c55.8-6.1,112,7.5,158.8,38.4h0c46.1,30.4,101.3,44.1,156.3,38.6l128.3-12.7v355.1Z"/>
    </svg>
`;